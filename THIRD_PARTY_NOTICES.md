...
BR-Log steht selbst unter der GNU General Public License v3.0 (GPLv3, siehe
[LICENSE](LICENSE)). Die Anwendung nutzt eine Reihe von Open-Source-
Komponenten unter eigenen Lizenzen (überwiegend MIT, Apache-2.0, BSD, ISC,
Zlib sowie einzelne permissive Sonderlizenzen). Diese Datei erfüllt die
Attributions-/Hinweispflichten dieser Lizenzen und listet alle direkten und
transitiven Laufzeitabhängigkeiten mit Name, Version und Lizenzangabe.

Keine der aufgeführten Komponenten ist unter einer mit der GPLv3
inkompatiblen Lizenz freigegeben. Einzelne Komponenten stehen unter der
Mozilla Public License 2.0 (MPL-2.0, dateibasiertes Copyleft) oder als
Alternative unter der LGPL-2.1-or-later – beides ist mit der GPLv3
kombinierbar.

## npm-Abhängigkeiten (Frontend, Produktivbetrieb)
...
Ermittelt mit `npx license-checker --production` (Stand: 2026-07-07, 33 Pakete).

## Rust-Abhängigkeiten (Tauri-Backend, `src-tauri`)
...
| sync_async | 0.1.0 | Apache-2.0 OR MIT |
...
| tauri-plugin-android-fs | 28.2.2 | Apache-2.0 OR MIT |
...
Ermittelt mit `cargo license` im Verzeichnis `src-tauri` (Stand: 2026-07-07, 525 Crates, projekteigene Crates `br-log` und `tauri-plugin-biometric-unlock` ausgenommen).

## Statisch eingebettete Komponenten

Zusätzlich zu den oben gelisteten Paketen bindet BR-Log über `rusqlite`
(Feature `bundled-sqlcipher-vendored-openssl`) zwei weitere Komponenten
statisch in die Anwendung ein, die nicht als eigenständige Cargo-/npm-Pakete
in Erscheinung treten:

- **SQLCipher** (Community Edition) – BSD-style Lizenz, © Zetetic LLC.
- **OpenSSL** – Apache License 2.0.

Beide Lizenztexte sind auf Anfrage bzw. in den jeweiligen Upstream-Repositories
einsehbar (https://www.zetetic.net/sqlcipher/ bzw. https://www.openssl.org/).
